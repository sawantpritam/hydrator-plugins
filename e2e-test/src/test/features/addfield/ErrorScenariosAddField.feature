...
Feature: AddField Plugin - Verify error scenarios
  ...
  Scenario: Validate invalid error messages in add field plugin without any input data
    ...
    And Verify that the Plugin Property: "asUUID" is displaying an in-line error message: "addFieldErrorMessageInvalidUUID"
  ...
  Scenario: Validate errors when field value is given and generate uuid as value is set to true
    ...
    And Verify that the Plugin Property: "asUUID" is displaying an in-line error message: "addFieldErrorMessageValidUUID"
