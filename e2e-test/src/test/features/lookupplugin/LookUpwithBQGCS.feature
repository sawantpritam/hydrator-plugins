...
Feature: LookUp - Verification of LookUp Plugin pipeline with BigQuery and GCS as source and  BigQuery as target
  ...
  Scenario: To verify complete flow of data extract and transfer from BigQuery and GCS as source to target BigQuery with LookUp plugin using GCS lookup dataset.
  ...
  @BQ_SOURCE_LOOKUP_TEST @GCS_CSV_LOOKUP_TEST @BQ_SINK_TEST
  Scenario:To verify complete flow of data extract and transfer from sources GCS and BigQuery to target BigQuery with LookUp plugin using BigQuery lookup dataset.
    Given Open Datafusion Project to configure pipeline
    When Select plugin: "GCS" from the plugins list as: "Source"
    When Expand Plugin group in the LHS plugins list: "Analytics"
    When Select plugin: "Lookup" from the plugins list as: "Analytics"
    ...
    Then Enter input plugin property: "lookUpOutputField" with value: "lookUpOutputField1"
  ...
  @BQ_SOURCE_LOOKUP_TEST @GCS_CSV_LOOKUP_TEST @BQ_SINK_TEST
  Scenario: To verify complete flow of data extract and transfer from sources GCS and BigQuery to target BigQuery with LookUp plugin without using Output field.
    Given Open Datafusion Project to configure pipeline
    When Select plugin: "GCS" from the plugins list as: "Source"
    When Expand Plugin group in the LHS plugins list: "Analytics"
    When Select plugin: "Lookup" from the plugins list as: "Analytics"
